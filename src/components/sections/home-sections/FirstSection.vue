    ...
    <div class="first-section__left-col">
      ...
      <ul class="first-section__social-list">
        <li
          ...
          :key="icon.name"
        >
          <a :href="icon.href" target="_blank" :class="icon.classLink">
...
.first-section {
  ...
  &__bg-title-container {
    position: relative;
    margin-bottom: 60px;
  }
  &__back-title {
    @include text(100px, 800, $bg-text-color);
    letter-spacing: 18px;
    line-height: 110px;
    margin-bottom: 30px;
    &._second {
  ...
  &__front-title {
    @include text(70px, 400, $light-color);
    letter-spacing: 18px;
    position: absolute;
    top: 50px;
    left: 113px;
    span {
      display: block;
      @include text(70px, 600, $light-color);
      top: 61px;
      left: -42px;
    }
  }
  &__subtitle {
    @include text($H200, 400, $blue-text-color);
    text-align: left;
    margin-bottom: 45px;
  }
  &__triangle {
